...
def process_audit(parsed_games, run_id, html_size_kb=0):
    ...
    """
    with tracer.start_as_current_span("process_audit") as span:
        span.set_attribute("run_id", run_id)
        span.set_attribute("input_games_count", len(parsed_games))
        ...
        span.set_attribute("old_wealth", old_wealth)
        span.set_attribute("new_wealth", new_wealth)
        
        # 1. Hard Check (Current vs. Immediately Previous)
        if old_wealth > 0 and new_wealth < (old_wealth * 0.75):
            msg = f"Hard Integrity Failure: Wealth dropped {((old_wealth-new_wealth)/old_wealth)*100:.1f}%. ABORTING."
            print(f"!!! [Notary] {msg}")
        ...
        if baseline:
            ...
            if wealth_diff > 0.40: # 40% deviation from mean is a red flag
                span.add_event("statistical_anomaly", {"wealth_diff": wealth_diff})
        ...
        span.set_attribute("birth_count", birth_count)
        span.set_attribute("death_count", death_count)
